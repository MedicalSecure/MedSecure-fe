...
export class VisitService implements ActivityService {
  ...
  getVisits(): Observable<any> {
    return this.http.get<any>('http://localhost:6004/visit-service/v1/visits');
  }
  //create visits
  creatVisits(formData: any) {
    const typeVisitEnumValue: TypeVisit =
      TypeVisit[formData.typevisits as keyof typeof TypeVisit];
    const locationVisitEnumValue: LocationVisit =
      LocationVisit[formData.disponibilite as keyof typeof LocationVisit];
    ...
    return this.http.post<any>(
      'http://localhost:6004/visit-service/v1/visits',
      VisitDtoWrapper
    );
  }
  ...
  deleteVisits(visitId: string | number | undefined) {
    return this.http.delete(
      `http://localhost:6004/visit-service/v1/visits/${visitId}?Id=${visitId}`
    );
  }
  ...
  getVisitsCountByToday(): Observable<number> {
    return this.getVisits().pipe(
      map((response) => {
        ...
        visits.forEach((visit: VisitDtoType) => {
          const visitDate = new Date(visit.startDate).toLocaleDateString(
            'en-CA'
          );
          if (visitDate === today) {
            visitCountToday++;
  ...
  getTotalVisitsCount(): Observable<number> {
    return this.getVisits().pipe(map((response) => response.visits.count));
  }
  ...
  getActivities(
    ...
    let x = this.http
      .get<GetActivitiesResponse>(
        'http://localhost:6004/visit-service/v1/visits' + '/Activities',
        {
          params,
        }
      )
